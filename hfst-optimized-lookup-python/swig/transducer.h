...
namespace hfst_ol {
    ...
    typedef std::vector<std::pair<std::string, Weight>> ResultVector;
...
// trie-tokenization
    ...
    private:
        ...
        static void header_error() { HFST_THROW(TransducerHasWrongTypeException); }

        template <class T> static T read_property(std::istream &is) {
          T p;
          is.read(reinterpret_cast<char *>(&p), sizeof(T));
          return p;
        }
        template <class T> static void write_property(T prop, std::ostream &os) {
    ...
    public:
        TransducerHeader(bool weights)
                : number_of_input_symbols(0), number_of_symbols(1), // epsilon
                  size_of_transition_index_table(1), size_of_transition_target_table(0),
                  number_of_states(1), number_of_transitions(0), weighted(weights),
                  deterministic(true), input_deterministic(true), minimized(true),
                  cyclic(false), has_epsilon_epsilon_transitions(false),
        ...
        TransducerHeader(SymbolNumber input_symbols, SymbolNumber symbols,
                         TransitionTableIndex transition_index_table,
                         TransitionTableIndex transition_table, bool weights)
                : number_of_input_symbols(input_symbols),
                  ...
                  size_of_transition_target_table(transition_table), number_of_states(0),
                  number_of_transitions(0), weighted(weights), deterministic(true),
                  input_deterministic(true), minimized(true), cyclic(false),
        ...
        TransducerHeader(std::istream &is)
                : number_of_input_symbols(read_property<SymbolNumber>(is)),
                  ...
                  weighted(read_bool_property(is)), deterministic(read_bool_property(is)),
                  input_deterministic(read_bool_property(is)),
                  minimized(read_bool_property(is)), cyclic(read_bool_property(is)),
        ...
        void display() const {
          std::cout << "Transducer properties:" << std::endl
                    ...
                    << " number_of_input_symbols: " << number_of_input_symbols
                    << std::endl
                    ...
                    << " has_input_epsilon_cycles: " << has_input_epsilon_cycles
                    << std::endl
                    << " has_unweighted_input_epsilon_cycles: "
    ...
    public:
        ...
        const hfst::FdTable<SymbolNumber> &get_fd_table() const { return fd_table; }
        const hfst::FdOperation *get_operation(SymbolNumber symbol) const {
          return fd_table.get_operation(symbol);
        }
        SymbolNumber get_unknown_symbol(void) const { return unknown_symbol; }
    };
    ...
    public:
        ...
        TransitionIndex()
                : input_symbol(NO_SYMBOL_NUMBER), first_transition_index(NO_TABLE_INDEX) {
        }
        TransitionIndex(SymbolNumber input, TransitionTableIndex first_transition)
        ...
        TransitionIndex(char *p)
                : input_symbol(*((SymbolNumber *)p)),
                  first_transition_index(
                          (*(TransitionTableIndex *)(p + sizeof(SymbolNumber)))) {}
        ...
        void write(std::ostream &os, bool weighted) const {
          ...
          if (!weighted and input_symbol == NO_SYMBOL_NUMBER and
            ...
            os.write(reinterpret_cast<const char *>(&first_transition_index),
                     sizeof(first_transition_index));
          }
        }
    ...
    public:
        ...
        TransitionWIndex(SymbolNumber input, TransitionTableIndex first_transition)
                : TransitionIndex(input, first_transition) {}
        TransitionWIndex(std::istream &is) : TransitionIndex(is) {}
        TransitionWIndex(char *p) : TransitionIndex(p) {}
    ...
    public:
        ...
        Transition(bool final, Weight bogus = 0.0f)
                : input_symbol(NO_SYMBOL_NUMBER), output_symbol(NO_SYMBOL_NUMBER),
                  target_index(final ? 1 : NO_TABLE_INDEX) {
          (void)bogus;
          bogus = 0.0f;
        }
    ...
    public:
        ...
        TransitionW(bool final, Weight w) : Transition(final), transition_weight(w) {}
        ...
        TransitionW(char *p)
                : Transition(p),
                  transition_weight(*((Weight *)(p + 2 * sizeof(SymbolNumber) +
                                                 sizeof(TransitionTableIndex)))) {}
    ...
    template <class T> class TransducerTable {
    protected:
        std::vector<T> table;
    ...
    public:
        ...
        TransducerTable(std::istream &is, TransitionTableIndex index_count)
                : table() {
        ...
        const T &operator[](TransitionTableIndex i) const {
          return (i < TRANSITION_TARGET_TABLE_START)
                 ? table[i]
                 : table[i - TRANSITION_TARGET_TABLE_START];
        }
    ...
    public:
        ...
        virtual TransitionTableIndex
        get_transition_target(TransitionTableIndex i) const = 0;
        ...
        virtual TransitionTableIndex
        get_index_target(TransitionTableIndex i) const = 0;
    ...
    public:
        TransducerTables(std::istream &is, TransitionTableIndex index_table_size,
                ...
                : index_table(is, index_table_size),
...
// There follow some classes for implementing lookup
    ...
    protected:
        ...
        void find_transitions(SymbolNumber input, SymbolNumber *input_symbol,
                              SymbolNumber *output_symbol,
                              SymbolNumber *original_output_tape,
                              TransitionTableIndex i);

        void find_index(SymbolNumber input, SymbolNumber *input_symbol,
                        SymbolNumber *output_symbol,
                        SymbolNumber *original_output_tape, TransitionTableIndex i);

        void get_analyses(SymbolNumber *input_symbol, SymbolNumber *output_symbol,
                          SymbolNumber *original_output_tape, TransitionTableIndex i);
    ...
    public:
        ...
        const Encoder &get_encoder(void) const { return *encoder; }
        ...
        get_transitions_from_state(TransitionTableIndex state_index) const;

        bool initialize_input(const char *input_str);
        std::vector<std::string> multi_lookup(const StringVector &strs);

        std::vector<std::vector<std::string> > multi_doc_lookup(const std::vector<std::string> &docs);
        ...
        bool is_weighted(void) { return header->probe_flag(Weighted); }
    ...
    public:
        ...
        STransition(TransitionTableIndex i, SymbolNumber s)
                : index(i), symbol(s), weight(0.0) {}

        STransition(TransitionTableIndex i, SymbolNumber s, Weight w)
                : index(i), symbol(s), weight(w) {}
    };
...
/* results are reversed by default because greater weights represent
   worse results - to reverse the reversal, give a true argument*/
    ...
    public:
        ...
        bool
        operator()(StringWeightPair lhs,
                   StringWeightPair
                   rhs) { // return true when we want rhs to appear before lhs
          if (reverse) {
    ...
    typedef std::priority_queue<StringWeightPair, std::vector<StringWeightPair>,
...
public:
    ...
    public:
        ...
        TreeNode(SymbolNumberVector prev_string, unsigned int i,
                 TransitionTableIndex mutator, TransitionTableIndex lexicon,
                 hfst::FdState<SymbolNumber> state, Weight w)
                : string(prev_string), input_state(i), mutator_state(mutator),
                  lexicon_state(lexicon), flag_state(state), weight(w) {}

        TreeNode(hfst::FdState<SymbolNumber> start_state)
                : // starting state node
                string(SymbolNumberVector()),
                input_state(0), mutator_state(0), lexicon_state(0),
                flag_state(start_state), weight(0.0) {}

        TreeNode update_lexicon(SymbolNumber next_symbol,
                                TransitionTableIndex next_lexicon, Weight weight);

        TreeNode update_mutator(SymbolNumber next_symbol,
                                TransitionTableIndex next_mutator, Weight weight);

        void increment_mutator(void);

        TreeNode update(SymbolNumber next_symbol, unsigned int next_input,
                        TransitionTableIndex next_mutator,
                        TransitionTableIndex next_lexicon, Weight weight);

        TreeNode update(SymbolNumber next_symbol, TransitionTableIndex next_mutator,
                        TransitionTableIndex next_lexicon, Weight weight);
    };
    ...
    public:
        InputString() : s(SymbolNumberVector()) {}

        bool initialize(const Encoder &encoder, char *input, SymbolNumber other);

        unsigned int len(void) { return s.size(); }

        SymbolNumber operator[](unsigned int i) { return s[i]; }
    };

    class AlphabetTranslationException
            : public std::runtime_error { // "what" should hold the first untranslatable
        // symbol
    public:
    ...
    public:
        ...
        Speller(Transducer *mutator_ptr, Transducer *lexicon_ptr)
                : mutator(mutator_ptr), lexicon(lexicon_ptr), input(InputString()),
                  queue(TreeNodeQueue()), alphabet_translator(SymbolNumberVector()),
